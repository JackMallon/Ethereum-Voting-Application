    ...
    app.candidatesCount().then(function( c ) {
      ...
      for (i = 1; i < count+1; i++) {
        const promise = app.candidates(i).then(function(c) {
          candidate = c;
          candidateObject = candidateModel.create(candidate.id, candidate.electionId, candidate.name, candidate.voteCount);
    ...
    app.electionsCount().then(function( c ) {
      ...
      for (i = 1; i < count+1; i++) {
        ...
        const promise = app.elections(i).then(function(e) {
          election = e;
          electionObject = electionModel.create(election.id, election.title, election.closingDate);
    ...
    app.electionVotesCount().then(function( c ) {
      ...
      for (i = 1; i < count+1; i++) {
        const promise = app.electionVotesById(i).then(function(e) {
          electionVote = e;
          electionObject = electionVoteModel.create(electionVote.id, electionVote.candidateId, electionVote.electionTitle, electionVote.candidateName, electionVote.userHash, electionVote.timestamp);
